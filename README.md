...
A mention inside a thread the plugin does not know yet binds that thread to a new task. Plain channel messages are ignored unless `chatRequireMention` is off. Slack's `message` + `app_mention` twins and retries are deduplicated by channel and timestamp. Direct messages are not supported yet: the bot says so and creates nothing.

### The tasks channel: every task gets a thread

Set `chatTasksChannelId` to a channel the bot is in, and the binding runs the other way too: a task that appears in Tandem (created in the web app, by an agent, from a routine) gets a thread in that channel, rooted at a card with its identifier, title, status and assignee and an Open button. The card is edited in place as the task moves (Queued, Working, Needs you, In review, Done); replies under it are comments on the task, the agent's comments come back into it, the run status and the ✅ line work as in any bound thread. Nothing else is posted in the channel for that task, and the "issue created" notification is skipped for it.

`chatTasksThreadScope` says which tasks qualify: `assigned` (the default) opens threads for top-level tasks that have an agent, `all` for every top-level task. Subtasks never get their own thread. A task that qualifies only later (assigned after creation) gets its thread on that update.

Config keys: `chatTasksEnabled`, `chatTasksProjectId`, `chatRequireMention`, `chatAckReaction`, `chatWorkingReaction`, `chatTasksChannelId`, `chatTasksThreadScope`, `chatPairings` (Slack user id → Tandem user id: a paired person's replies are their own comments and wake the assignee the way a web comment does; everyone else is relayed under the bot's name). Required Slack bot events: `app_mention`, `message.channels`, `message.groups`, `message.im`, `message.mpim`; scopes: `app_mentions:read`, `channels:history`, `groups:history`, `im:history`, `mpim:history`, `chat:write`, `reactions:write`. `slack-app-manifest.json` in this repository is a complete app manifest to paste into Slack.
